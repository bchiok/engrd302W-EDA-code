...
avghealthcare_2022 <- read.csv("data/policymaptest.csv") %>% 
...
covidcases2021 <- read.csv("data/us-counties-2021.csv") %>% 
  ...
  pivot_wider(names_from = "date",
              values_from = "cases") %>% 
  rowwise() %>%
  mutate(covidcases_2021 = sum(c_across(starts_with("2021")), na.rm = T)) %>%
...
covidcases2022 <- read.csv("data/us-counties-2022.csv") %>% 
  ...
  pivot_wider(names_from = "date",
              values_from = "cases") %>% 
  rowwise() %>%
  mutate(covidcases_2022 = sum(c_across(starts_with("2022")), na.rm = T)) %>%
...
covid_costs <- avghealthcare_2022 %>% 
  left_join(covidcases2021, by = c("county","state", "fips")) %>% 
